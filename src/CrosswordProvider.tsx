...
>(
  (
    ...
    const findEdgeClue = useCallback((direction:Direction, front:boolean) => {
      ...
      const clueNumbers = clues?.[direction].map((clue) => clue.number).sort((a, b) => parseInt(a) - parseInt(b));
      console.log('clue numbers');
      console.log(clueNumbers);
      if (front) {
        return clues?.[direction].find((clue) => clue.number === clueNumbers[0]);
      } else {
        return clues?.[direction].find((clue) => clue.number === clueNumbers[clueNumbers.length - 1]);
      }
    }, [clues]);

    const findClosestClueToCurrent = useCallback((forwards:boolean) => {
      if (!clues) {
        return null;
      }
